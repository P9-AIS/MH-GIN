...
def filter_by_draught(group, sort_num=2):
    unique_draughts = group['Draught'].nunique()
...
def process_dataframe(df, chunk_size=2, draught_count_diffs=2000):
    ...
    for mmsi, group in grouped:
        ...
        for chunk in draught_chunks:
            ...
            draught_counts = group[group['Draught'].isin(chunk)].groupby('Draught').size()
            ...
            valid_group = group[group['Draught'].isin(chunk)]

            mmsi_id = f"{mmsi}_{valid_group_count}"
            traj_data[mmsi_id] = {
                "vessel_type": valid_group['Ship type'].tolist(),
                "vessel_width": valid_group['Width'].tolist(),
                "vessel_length": valid_group['Length'].tolist(),
                "vessel_draught": valid_group['Draught'].tolist(),
                "destination": valid_group['Destination'].tolist(),
                "cargo_type": valid_group['Cargo type'].tolist(),
                "navi_status": valid_group['Navigational status'].tolist(),
...
def process_ais_multi_csv_dataset(args):
    ...
    traj_data = split_trajectory2(traj_data, min_len=int(args.min_traj_len), max_len=max_traj_len, min_time_threshold=int(args.min_time_interval),
...
def hyperparameter_DataProcess(parser):
    parser.add_argument("--config", type=str,
                            default=f'{project_root}/config/config-DK.yaml',
                            help='Path to configuration file')
    # Dataset Process
    # default=['aisdk-2024-03-01@31', 'aisdk-2024-04-01@30', 'aisdk-2024-05-01@31']
    # default=['aisdk-2024-06-01@30', 'aisdk-2024-07-01@31', 'aisdk-2024-08-01@31']
    # default=['aisdk-2024-09-01@30', 'aisdk-2024-10-01@31', 'aisdk-2024-11-01@30']
    parser.add_argument("--datasets", type=list,
                        default=['aisdk-2024-03-01@31', 'aisdk-2024-04-01@30', 'aisdk-2024-05-01@31',
                                 'aisdk-2024-06-01@30', 'aisdk-2024-07-01@31'])
